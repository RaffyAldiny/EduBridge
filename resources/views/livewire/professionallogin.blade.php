...
<div x-show="showProfessionalModal" x-cloak 
    ...
    @if (session()->has('error'))
        ...
        <form x-show="isLogin" wire:submit.prevent="login" class="space-y-1">
            ...
            <div class="mb-1">
                <input type="password" wire:model="password" class="w-full p-[2px] placeholder-hover border-[2px] border-hover text-hover rounded-[2px] focus:bg-whitehover focus:border-[3px] focus:outline-none duration-100" placeholder="Password" required>
            </div>
        ...
        <!-- Sign-up Form -->
        <form x-show="!isLogin" wire:submit.prevent="signup" class="space-y-1">
            <div class="mb-3">
            ...
            <label for="file-upload-professional" class="cursor-pointer flex flex-col items-center justify-center border-[2px] border-solid border-hover rounded-[3px] h-14 hover:bg-whitehover duration-100">
                ...
                <input id="file-upload-professional" type="file" wire:model="resume" class="hidden" @change="fileNameProfessional = $event.target.files[0].name" required>
            </label>
